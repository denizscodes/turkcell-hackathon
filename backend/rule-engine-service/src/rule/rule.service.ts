...
export class RuleService {
  ...
  async update(id: string, updateRuleDto: any): Promise<Rule> {
    ...
    if (updateRuleDto.eventType) updateData.event_type = updateRuleDto.eventType
    if (updateRuleDto.conditions) updateData.conditions = updateRuleDto.conditions
    ...
    if (updateRuleDto.isActive !== undefined) updateData.is_active = updateRuleDto.isActive

    const { data, error } = await supabase.from("rules").update(updateData).eq("id", id).select().single()
  ...
  async delete(id: string): Promise<Rule> {
    const { data, error } = await supabase.from("rules").delete().eq("id", id).select().single()
  ...
  async getStats() {
    const { count: total } = await supabase.from("rules").select("*", { count: "exact", head: true })
